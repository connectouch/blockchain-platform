...
const CrossChainAnalytics: React.FC<CrossChainAnalyticsProps> = ({
  ...
  const [supportedChains] = useState<ChainConfig[]>([])
  const [chainMetrics] = useState<{ [chainId: string]: ChainMetrics }>({})
  const [crossChainPortfolio] = useState<CrossChainPortfolio | null>(null)
  const [defiProtocols] = useState<{ [chainId: string]: any[] }>({})
  const [gasPrices] = useState<{ [chainId: string]: any }>({})
  const [isLoading] = useState(false)
  const [selectedChains] = useState<string[]>([])
  const [showAllChains] = useState(true)
