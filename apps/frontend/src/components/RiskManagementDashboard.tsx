...
const RiskManagementDashboard: React.FC<RiskManagementDashboardProps> = ({
  ...
  // Initialize risk data with real market data
  useEffect(() => {
    setIsLoading(true)
    ...
    const fetchRealRiskData = async () => {
      try {
        ...
        // Calculate real risk metrics based on market data
        const avgVolatility = prices.reduce((sum, p) => sum + Math.abs(p.change24h), 0) / prices.length;
        const marketRisk = Math.abs(marketData.marketCapChange24h);

        const realRiskMetrics: RiskMetrics = {
          portfolioRisk: avgVolatility > 5 ? 'high' : avgVolatility > 2 ? 'medium' : 'low',
          riskScore: Math.min(100, Math.max(0, 50 + avgVolatility * 5)),
          volatility: avgVolatility,
          sharpeRatio: Math.max(0, 2 - avgVolatility / 10),
          maxDrawdown: -Math.max(10, avgVolatility * 2),
          var95: portfolioValue * (0.05 + avgVolatility / 100),
          var99: portfolioValue * (0.10 + avgVolatility / 50),
          expectedShortfall: portfolioValue * (0.15 + avgVolatility / 30),
          beta: 0.5 + (avgVolatility / 20),
          correlationRisk: Math.min(100, 40 + marketRisk * 10),
          concentrationRisk: Math.min(100, 30 + avgVolatility * 5),
          liquidityRisk: Math.min(100, 20 + marketRisk * 3),
          counterpartyRisk: Math.min(100, 15 + avgVolatility * 2)
        }
      ...
      const mockLiquidity: LiquidityAnalysis[] = [
        ...
        const fallbackRiskMetrics: RiskMetrics = {
