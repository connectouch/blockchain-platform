...
Please provide analysis based on this real-time market data.`
      ...
      const response = await ApiService.sendAIChat(enhancedMessage, { conversationHistory, type: 'general' })
  ...
  const handleVoiceMessage = async (voiceText: string) => {
    ...
    try {
      const response = await ApiService.sendAIChat(voiceText, { conversationHistory: [], type: 'general' })
      const aiResponse: Message = {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
